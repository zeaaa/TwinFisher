...
public class suimono_shorelineobject_editor : Editor {
    ...
    Undo.RecordObject(target, "Changed Area Of Effect");
		...
		//bgPresetNd = Resources.Load("textures/gui_bgpresetNd") as Texture;
			...
			logoTex = Resources.Load("textures/gui_tex_suimonologoshore_i") as Texture;
        ...
        script.shorelineModeIndex = EditorGUI.Popup(new Rect(rt.x+margin+130, rt.y+60, 100, 18),"", script.shorelineModeIndex, script.shorelineModeOptions.ToArray());
  		...
  		if (script.shorelineModeIndex == 0){
			...
			EditorGUI.LabelField(new Rect(rt.x+margin+5, rt.y+120, 130, 18),"Depth Range(R)");
			script.sceneDepth = EditorGUI.Slider(new Rect(rt.x+margin+130, rt.y+120, setWidth+35, 18),"",script.sceneDepth,0.0f,50.0f);

			EditorGUI.LabelField(new Rect(rt.x+margin+5, rt.y+140, 130, 18),"Shore Range(G)");
			script.shoreDepth = EditorGUI.Slider(new Rect(rt.x+margin+130, rt.y+140, setWidth+35, 18),"",script.shoreDepth,0.0f,50.0f);

			
			EditorGUI.LabelField(new Rect(rt.x+margin+5, rt.y+basePos+160, 180, 18),"Calculate Layers");
			if (script.moduleObject != null){
				script.depthLayer = EditorGUI.MaskField(new Rect(rt.x+margin+130, rt.y+basePos+160, 90, 18),"", script.depthLayer, script.suiLayerMasks.ToArray());
			}

...
public class suimono_object_editor : Editor {
    ...
    if (script.editorIndex == 1){
        ...
        if (script.showSurface){
        	...
        	EditorGUI.LabelField(new Rect(rt.x+margin+10, rt.y+395, 130, 18),"Reflection Color");
            script.reflectionColor = EditorGUI.ColorField(new Rect(rt.x+margin+165, rt.y+395, setWidth, 18),"",script.reflectionColor);

			
			EditorGUI.DrawPreviewTexture(new Rect(rt.x+margin+10,rt.y+420,372,1),divHorizTex);


			EditorGUI.LabelField(new Rect(rt.x+margin+10, rt.y+430, 140, 18),"Hot Specular");
			script.roughness = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+430, setWidth, 18),"",script.roughness,0.0f,1.0f);
			EditorGUI.LabelField(new Rect(rt.x+margin+10, rt.y+450, 140, 18),"Wide Specular");
			script.roughness2 = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+450, setWidth, 18),"",script.roughness2,0.0f,1.0f);
        	EditorGUI.LabelField(new Rect(rt.x+margin+10, rt.y+470, 130, 18),"Specular Color");
            script.specularColor = EditorGUI.ColorField(new Rect(rt.x+margin+165, rt.y+470, setWidth, 18),"",script.specularColor);
        	EditorGUI.LabelField(new Rect(rt.x+margin+10, rt.y+490, 130, 18),"Back Light Scatter");
            script.sssColor = EditorGUI.ColorField(new Rect(rt.x+margin+165, rt.y+490, setWidth, 18),"",script.sssColor);
        ...
        if (script.showPresets){
			...
			//select preset file
			EditorGUI.LabelField(new Rect(rt.x+margin+18, rt.y+24, 110, 18),"Use Preset File:");
			script.presetFileIndex = EditorGUI.Popup(new Rect(rt.x+margin+125, rt.y+24, 258, 13),"",script.presetFileIndex, script.presetDirs.ToArray());
    GUI.contentColor = colorDisabled;
    GUI.backgroundColor = colorDisabled;
			EditorGUI.LabelField(new Rect(rt.x+margin+18, rt.y+44, 100, 18),"Transition:");
			script.presetTransIndexFrm = EditorGUI.Popup(new Rect(rt.x+margin+85, rt.y+44, 80, 13),"",script.presetTransIndexFrm, script.presetOptions);
			EditorGUI.LabelField(new Rect(rt.x+margin+167, rt.y+44, 100, 18),"-->");
			script.presetTransIndexTo = EditorGUI.Popup(new Rect(rt.x+margin+194, rt.y+44, 80, 13),"",script.presetTransIndexTo, script.presetOptions);
